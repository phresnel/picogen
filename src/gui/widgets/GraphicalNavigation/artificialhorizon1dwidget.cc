...
#include "artificialhorizon1dwidget.hh"
#include <QPainter>
#include <QBrush>
#include <QPen>
...
ArtificialHorizon1DWidget::ArtificialHorizon1DWidget(QWidget *parent)
        : QWidget(parent)
{
        //setFlags (QGraphicsItem::ItemIsMovable);
}

/*QRectF ArtificialHorizon1DWidget::boundingRect() const {
        QRectF ret;
        ret.setLeft(-80);
        ret.setRight(80);
        ret.setBottom(80);
...
int ArtificialHorizon1DWidget::radius() const {
        return rect().width()<rect().height() ?
               rect().width()/2-30 :
               rect().height()/2-30;
}

QPointF ArtificialHorizon1DWidget::center() const {
        return rect().center();
}

void ArtificialHorizon1DWidget::drawArtificialHorizon(QPainter *painter) {
...
void ArtificialHorizon1DWidget::paintEvent(QPaintEvent *) {
...
void ArtificialHorizon1DWidget::mousePressEvent(QMouseEvent *event) {
        updateIndicator = update_roll;
        updateIndicators(event);
        event->accept();
}

void ArtificialHorizon1DWidget::mouseMoveEvent(QMouseEvent *event) {
        updateIndicators(event);
        event->accept();
}

void ArtificialHorizon1DWidget::mouseReleaseEvent(QMouseEvent *event) {
        updateIndicators(event);
        updateIndicator = update_none;
        event->accept();
}

void ArtificialHorizon1DWidget::updateIndicators(QMouseEvent *event) {
