...
private:
    ...
    template <typename T> void SetYprOrientation (T yaw, T pitch, T roll) const {        
        ...
        ypr_yaw  ->SetValue (round <int> (yaw));
        ypr_pitch->SetValue (round <int> (pitch));
        ypr_roll ->SetValue (round <int> (roll));
        
        ypr_yaw_fine  ->SetValue (static_cast <int> (0.5 + 100.0 * (yaw - round <T> (yaw))));
        ypr_pitch_fine->SetValue (static_cast <int> (0.5 + 100.0 * (round <T> (pitch) - pitch)));
        ypr_roll_fine ->SetValue (static_cast <int> (0.5 + 100.0 * (round <T> (roll) - roll)));
    }
