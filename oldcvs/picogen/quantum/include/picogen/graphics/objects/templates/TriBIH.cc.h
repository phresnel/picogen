...
// This file has been forked from graphics/objects/TriBIH.cc
...
namespace picogen {
    namespace graphics {
        namespace objects {
            namespace templates {
                ...
                TriBIH_Member(int)::raytri_intersect (
                    const ::picogen::common::Ray &ray,
                    const ::picogen::common::Vector3d &a,
                    const ::picogen::common::Vector3d &b,
                    const ::picogen::common::Vector3d &c,
                    ::picogen::common::real &t,
                    ::picogen::common::real &u,
                    ::picogen::common::real &v,
                    ::picogen::common::Vector3d &normal
                ) const {
                ...
                TriBIH_Member(void)::setBRDF (const IBRDF *BRDF) {
                    this->BRDF = BRDF;
                }



                TriBIH_Member(void)::setShader (const material::abstract::IShader *shader) {
                    this->shader = shader;
                }



                TriBIH_Member(bool)::intersect (
                    ...
                    if (node->is_leaf) {
                        ...
                        unsigned int i;
                        bool any = false;
                        for (i=node->triangles_begin; i<node->triangles_end; i++) {
                            real t,u,v;
                            Vector3d N;
                            // itnersect ray  with triangle
                            const int tmp = raytri_intersect (ray, Triangles[i][0], Triangles[i][1], Triangles[i][2], t,u,v,N);
                ...
                TriBIH_Member(bool)::intersect (param_out (intersection_t,intersection), param_in (Ray,ray)) const {
                    using namespace misc::constants;
                    real t_min, t_max;
                ...
                TriBIH_Member(void)::insert (param_in (Vector3d,A), param_in (Vector3d,B), param_in (Vector3d,C)) {
                    AABB.update (A);
                    AABB.update (B);
                    AABB.update (C);
                ...
                TriBIH_Member(void)::bih_build_node::insert (param_in (t_triangle,tri)) {
                    aabb.update (tri[0]);
                    aabb.update (tri[1]);
                    aabb.update (tri[2]);
                ...
                TriBIH_Member(void)::bih_build_node::free() {
                    aabb.reset();
                    triangles.clear();
                    if (0 != p_left)
                        delete p_left;
                ...
                TriBIH_Member(bool)::bih_build_node::rec_build (const BoundingBox &build_aabb, const unsigned int maxRec=32) {

    #if TPL_TriBIH_DebugBIHBuild
                    if (maxRec == 0)
                    ...
                    if (maxRec>0 && triangles.size() > 4) {   // then split
                        ...
                        do {
                            ...
                            p_left->free();
                            p_right->free();
                            t_trianglelist_const_iterator it;
                            for (it=triangles.begin(),count_L=count_R=0; it!=triangles.end(); it++) {   // for each triangle
                                const real center = (1.0/3.0) * ((*it)[0][axis] + (*it)[1][axis] + (*it)[2][axis]);
                ...
                TriBIH_Member(unsigned int)::bih_build_node::num_nodes() const {
                ...
                TriBIH_Member(unsigned int)::bih_build_node::num_triangles() const {
                    if (0 != p_left && 0 != p_right) {
                ...
                TriBIH_Member(void)::bih_build_node::rec_convert (
                    //>
    ...
    #if TPL_TriBIH_DebugBIHRecConvert
                    {
                        unsigned int i;
                        for (i=0; i<depth; i++)
                            fprintf (stderr, ".");
                ...
                TriBIH_Member(bool)::bih_build_node::build (bih_node **bih, t_triangle **triangles) {
                    using namespace misc::constants;
