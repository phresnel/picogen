...
namespace quatsch {  namespace backend {  namespace est {
    ...
    {
        ::boost::shared_array <FunctionPtr> functions (new FunctionPtr [this->functions.size()]);
        int i=0;
        if (debug) {
            ::std::cout << "declared functions: " << ::std::endl;
        }
        for (typename SymbolTable <FunctionDescriptor<Backend> >::const_iterator it=this->functions.begin(); it!=this->functions.end(); ++it) {
            functions [i++] = it->second.getFunction();
            if (debug) {
                ::std::cout << "  " << it->first << ::std::endl;
            }
        }
        return node_types<Backend<R,P> >::RootFunction::create (operands.back().back(), functions);
    }
    ...
    Backend<R,P> :: ~Backend () {
        ...
        if (debug) {
            if (0 < functions.size()) {
                ...
                for (typename ::quatsch::SymbolTable <FunctionDescriptor<Backend> >::const_iterator it=functions.begin(); it != functions.end(); ++it) {
                    ::std::cout << "\t" << it->second.getName() << ":" << it->second.getParameterCount() << ":" << it->second.getFunction().get() << "\n";
                }
            }
        }
    }
        ...
        , syntaxError (*this)
    {
        ...
        //::std::cout << "yo' lousy configurableFunctionsMap has " << this->configurableFunctions.size() << " elements." << ::std::endl;
    }



    template <typename R, typename P>
    void Backend<R, P> :: dumpErrorMessages () const {
        using namespace std;
        bool valid = true;                                        
        ...
        //  After we are done with parsing, we can check whether all forward declared functions (or 'incomplete calls') have
        //  been resolved. Luckily FunctionDescriptor saved all calls to a function until it would be defined.
        //  We must simply call FunctionDescriptor::probe() to provoke an exception.
        if (0 < functions.size()) {
            bool all_forward_declarations_resolved = true;
            for (typename ::quatsch::SymbolTable <FunctionDescriptor<Backend> >::const_iterator it=functions.begin(); it != functions.end(); ++it) {
                if (!it->second.isComplete()) {
                    const ::std::string message ("call to unknown function \"" + it->second.getName() + "\".");
                    ::std::vector <SourcecodePosition> positions;
                    it->second.pushIncompleteCallPositions (positions);
                    for (typename ::std::vector <SourcecodePosition>::const_iterator pp=positions.begin(); pp!=positions.end(); ++pp) {
                        errorMessages.push_back (ErrorMessage (ErrorMessage::error, message, *pp));
                    }
        ...
        for (typename ::std::vector<ErrorMessage>::const_iterator it=errorMessages.begin(); it != errorMessages.end(); ++it) {
            if (it->isWarning()) {
                ...
                cerr << right << it->getColumnNumber();
                cerr.width (0);
                if (it->isCritical()) cerr << "): critical error: " << it->getMessage() << '\n';
                else cerr << "): error: " << it->getMessage() << '\n';
                valid = false;
            }
        }
