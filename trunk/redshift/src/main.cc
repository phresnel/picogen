...
namespace redshift {
        bool doesIntersect (Ray const &ray, Sphere const & sphere) {
typedef float real_t;


                const Vector v = kallisto::vector_cast<Vector> (ray.position - sphere.center);
                const real_t b =
...
public:
        ...
        : width (width), height (height) {
                ...
                planets.push_back (
                        PreviewPlanet (
                                CartesianPoint(
                                        kallisto::scalar_cast<fixed_point_t>(
                                          constants::earth_radius_m),
                                        kallisto::scalar_cast<fixed_point_t>(0.0),
                                        kallisto::scalar_cast<fixed_point_t>(
                                                constants::jupiter_radius_m * 1.75)
                                ),
                                constants::earth_radius_m,
                                Color(0.35,0.75,0.5)
                        )
