...
float Reporter::getCurrentPercentage () const {
...
void Reporter::report (redshift::RenderTarget::ReadLockPtr sourcel,
        ...
        currentPercentage = static_cast<float>(completed)
...
public:
        void render (int /*width*/, int /*height*/,
                ...
                //    i mean, a "RenderTarget" might be flipable, but a Film
                //    not, or so
...
SceneDisplayThread::~SceneDisplayThread () {
        ...
        if (commandProcessor.get())
                ((PassiveCommandProcessor*)commandProcessor.get())
                        ->setQuitState (true);
        this->wait();
}
...
void SceneDisplayThread::run() {
        ...
        this->renderBuffer = redshift::RenderTarget::Ptr(
                                new redshift::ColorRenderTarget(width,height));
...
: QWidget (parent), mustReRender (false)
{
        ...
        renderThread.setDisplaySubject(
               redshift::shared_ptr<SceneDisplaySubject> (new DummySubject ()));
}
