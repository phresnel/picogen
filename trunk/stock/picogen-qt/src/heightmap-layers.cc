...
public:
        ...
        void render (int /*width*/, int /*height*/,
                     ...
                     redshift::shared_ptr<redshift::interaction::
                                          ProgressReporter>  reporter,
                     redshift::interaction::UserCommandProcessor::Ptr
                                          commandProcessor
        ) {
                using namespace redshift;
                using namespace redshift::camera;
...
HeightmapLayersImpl::HeightmapLayersImpl(
        QMdiArea *mdiArea_, redshift::shared_ptr<scene::heightmap::Composition> composition_
)
: mdiArea (mdiArea_)
, composition (composition_)
{

        //composition->setSyncObject (shared_from_this());
...
HeightmapLayersImpl::~HeightmapLayersImpl() {
        composition->resetSyncObject();
        if (0 != display) {
                //delete display;
        }
        // remove remaining hard-locks
        for (std::map<int, QWidget*>::iterator it=openDefinitionWindows.begin();
                it != openDefinitionWindows.end();
                ++it
        ) {

                QMdiSubWindow *mdiSubWindow = (QMdiSubWindow *)(*it).second;

                QuatschEditorImpl *qe = dynamic_cast<QuatschEditorImpl*>(mdiSubWindow->widget());
                std::cout << "is quatch editor?" << std::endl;
                if (0 != qe) {
                        std::cout << "yes, is quatch editor" << std::endl;
                        disconnect(
                                qe, SIGNAL(storeRowParameters (int,
                                   scene::heightmap::RowParametersMerger const &)),
                                this, SLOT(storeRowParameters (int,
                                   scene::heightmap::RowParametersMerger const &))
                        );
                        disconnect(qe, SIGNAL(closingDefinitionWindow (int)),
                        ...
                        redshift::optional<scene::heightmap::RowData> row = composition->getRowById (id);
                        if (row) {
                                std::cout << row->id << ":hardlock " << row->hardLock << ":locked " << row->locked << std::endl;
                                row->hardLock = false;
                                std::cout << "hardlock removed" << std::endl;
                                composition->setRowById (id, *row);
                        }
                        std::cout << "sync'ed" << std::endl;
                }
        }
        ...
        // remove definition windows
        for (std::map<int, QWidget*>::iterator it=openDefinitionWindows.begin();
                it != openDefinitionWindows.end();
                ++it
        ) {
                mdiArea->removeSubWindow((*it).second);
        }
}
...
redshift::shared_ptr<const scene::heightmap::Composition> HeightmapLayersImpl::getComposition() const {
        return composition;
}
...
void HeightmapLayersImpl::fillCompositionTypeComboBox (QComboBox *box) const {
        ...
        box->addItem ("Add", QVariant(scene::heightmap::Addition));
        box->addItem ("Subtraction", QVariant(scene::heightmap::Subtraction));
        box->addItem ("Multiplication", QVariant(scene::heightmap::Multiplication));
        box->addItem ("Division", QVariant(scene::heightmap::Division));
        box->addItem ("Lerp", QVariant(scene::heightmap::Lerp));
}
...
void HeightmapLayersImpl::swapRows(int rowA, int rowB)
{
        ...
        if (rowA>=0 && (size_t)rowA < composition->data.size()
            && rowB>=0 && (size_t)rowB < composition->data.size()
                ...
                scene::heightmap::RowData tmp = composition->data [rowA];
...
void HeightmapLayersImpl::storeRowParameters (
        int rowId, scene::heightmap::RowParametersMerger const &params
) {
        redshift::optional<scene::heightmap::RowData> data =
                                composition->getRowById (rowId);
        if (data) {
...
scene::heightmap::RowData HeightmapLayersImpl::rowFromView (int row) const {
        if (0>row)
                return scene::heightmap::RowData();
        ...
        // > same for hardLock
        redshift::optional<scene::heightmap::RowData> rp = composition->getRowById (id);
        scene::heightmap::RowParameters params;
        bool hardLock = false;
        ...
        return scene::heightmap::RowData(
                ...
                static_cast<scene::heightmap::RowData::Type>(
                        typeComboBox->itemData (
                        typeComboBox->currentIndex()).toInt()),

                table->item (row, Indices::Name)->text().toStdString(),
...
void HeightmapLayersImpl::rowToView (int rowIndex, scene::heightmap::RowData data) {
        //std::cout << data.id << ":hardlock " << data.hardLock << ":locked " << data.locked << std::endl;
        ...
        // Type
        {
                ...
                if (data.type < scene::heightmap::RowData::Group) {
                        box->setInsertPolicy (QComboBox::InsertAtBottom);
                        box->addItem ("1st Class", QVariant(scene::heightmap::RowData::FirstClass));
                        box->addItem ("Composition", QVariant(scene::heightmap::RowData::Composition));
                        box->addItem ("Bitmap", QVariant(scene::heightmap::RowData::Bitmap));
                        box->addItem ("Code", QVariant(scene::heightmap::RowData::Code));
                } else {
                        box->addItem ("Add", QVariant(scene::heightmap::RowData::GroupAddition));
                        box->addItem ("Subtract", QVariant(scene::heightmap::RowData::GroupSubtraction));
                        box->addItem ("Multiply", QVariant(scene::heightmap::RowData::GroupMultiplication));
                        box->addItem ("Divide", QVariant(scene::heightmap::RowData::GroupDivision));
                        box->addItem ("Lerp", QVariant(scene::heightmap::RowData::GroupLerp));
                }
                box->setEnabled (!locked);
        ...
        // Name
        {
                QTableWidgetItem *item = new QTableWidgetItem(QString::fromStdString(data.name));
...
void HeightmapLayersImpl::colorizeGroups () {
        ...
        for (int i=0; i<table->rowCount(); ++i) {
                const scene::heightmap::RowData::Type type = composition->data[i].type;
                if (type >= scene::heightmap::RowData::Group) {
                        ++colorId;
                        colorId %= colorCount;
                }
...
void HeightmapLayersImpl::on_openDefinition_clicked() {
        ...
        syncFromView();
        const scene::heightmap::RowData data = composition->data [row];//getRowData (row);
        ...
        switch (data.type) {
        case scene::heightmap::RowData::FirstClass:
                std::cout << "must. open. presets-dialog." << std::endl;
                break;
        case scene::heightmap::RowData::Bitmap:
                std::cout << "must. open. bitmap-dialog." << std::endl;
                break;
        case scene::heightmap::RowData::Composition: {
        ...
        case scene::heightmap::RowData::Code: {
                QuatschEditorImpl *ed = new QuatschEditorImpl (
                        QString::fromStdString(data.parameters.code), data.id);
                ed->setWindowTitle (QString::fromStdString(data.name));
                connect(
        ...
        case scene::heightmap::RowData::GroupAddition:
                break;
        case scene::heightmap::RowData::GroupSubtraction:
                break;
        case scene::heightmap::RowData::GroupMultiplication:
                break;
        case scene::heightmap::RowData::GroupDivision:
                break;
        case scene::heightmap::RowData::GroupLerp:
                break;
        };
}
...
void HeightmapLayersImpl::on_newLayer_clicked() {
        ...
        const scene::heightmap::RowData newRow = scene::heightmap::RowData (composition->generateId(),
                                true, false, false,  // visible, lock, hardLock
                                scene::heightmap::RowData::FirstClass, "New Layer");
...
void HeightmapLayersImpl::on_newGroup_clicked() {
        ...
        const scene::heightmap::RowData newRow = scene::heightmap::RowData (composition->generateId(),
                                true, false, false, // visible, lock, hardLock
                                scene::heightmap::RowData::GroupAddition, "New Group");
...
redshift::tuple<QString, QString> HeightmapLayersImpl::generateJuxCode (
        ...
        using namespace scene::heightmap;
        ...
        while (rowIndex<composition->data.size()) {
                ...
                case RowData::Code:
                        prolog += QString("\n")
                               + "//----------------------------------------\n"
                               + "// code for \"" + QString::fromStdString(data.name) + "\"\n"
                               + QString::fromStdString(params.code).replace (
...
QString HeightmapLayersImpl::generateJuxCode(
        ...
        redshift::shared_ptr<const scene::heightmap::Composition> composition, int /*recDepth*/
) const {
        ...
        for (size_t i=0; i<composition->data.size(); ++i) {
                redshift::tuple<QString,QString> t =
                        generateJuxCode(composition,namespaceMaker,1+indent_,i);
...
void HeightmapLayersImpl::syncFromView () {
        ...
        composition->type = (scene::heightmap::CompositionType)compositionType->
